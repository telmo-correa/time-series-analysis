McLeod.Li.test <- function (object, y, gof.lag, col = "red", omit.initial = TRUE, 
    plot = TRUE, ...) {
    BL = function(x, lag) {
    ...
    if (!missing(object)) {
        ...
        if (missing(y)) 
            stop("supply the time series thru the y-argument")
        residuals = y
    }
    n = length(residuals)
    if (missing(gof.lag)) 
    ...
    lbv = rep(NA, lag.max)
    for (i in 1:lag.max) {
        lbv[i] = BL(residuals^2, lag = i)
    }
    if (plot) {
        plot(y = lbv, x = 1:lag.max, ylim = c(0, 1), pch = 21, 
